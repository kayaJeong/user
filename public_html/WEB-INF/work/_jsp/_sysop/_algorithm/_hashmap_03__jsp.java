...
public class _hashmap_03__jsp extends com.caucho.jsp.JavaPage
{
...
p.setVar("SYS_TITLE", "[\uad00\ub9ac\uc790] enter \uc624\ud508 \ub354 \uc774\ub7ec\ub2dd \uc0ac\uc774\ud2b8");
    ...
    if(m.isPost()){
        ...
        int [] int1arr = new int[str1arr.length];
        for(int i = 0; i < str1arr.length; i++) {
            int1arr[i] = Integer.parseInt(str1arr[i]);
        }

        int str2 = Integer.parseInt(f.get("str2"));


        HashMap<Integer, Integer> map = new HashMap();
        //\ud574\uc2dc\ub9f5 \uccab\ubc88\uc9f8 \ucd08\uae30\uac12
        for(int i = 0; i < str2-1; i++){
            map.put(int1arr[i], map.getOrDefault(int1arr, 0 ) +1);
        }
        ...
        int pointer2 = 0;
        while(num >= str2 ){
            for(int pointer1 = str2; pointer1 <str1arr.length; pointer1++){
                map.put(int1arr[pointer1], map.getOrDefault(int1arr[pointer1], 0 ) +1);
                result += Integer.toString(map.size());
                map.put(int1arr[pointer2], map.get(pointer2)-1);
                if(map.get(int1arr[pointer2]) == 0) {
                    map.remove(int1arr[pointer2]);
                }
                pointer2++;
            }
            num++; 
    ...
    throws javax.servlet.ServletException
  {
    ...
    depend = new com.caucho.vfs.Depend(appDir.lookup("sysop/algorithm/hashmap_3.jsp"), -4352490853664284370L, false);
    com.caucho.jsp.JavaPage.addDepend(_caucho_depends, depend);
